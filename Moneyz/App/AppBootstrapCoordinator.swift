...
final class AppBootstrapCoordinator: ObservableObject {
    ...
    func bootstrapIfNeeded(
        ...
        applyRecurringTransactions(on: referenceDate, in: context, reason: "initial bootstrap")
    ...
    func handleScenePhase(
        ...
        if phase == .active {
            applyRecurringTransactions(on: referenceDate, in: context, reason: "scene became active")
        }
        appLock.handleScenePhase(phase, settings: settings)
    }

    private func applyRecurringTransactions(on referenceDate: Date, in context: ModelContext, reason: String) {
        do {
            let appliedCount = try recurringService.applyDueRules(on: referenceDate, in: context)
